...
def registration():
    ...
    return render_template('registration.html', form=form, catregories=categories)
...
def login():
    ...
    categories = Category.query.all()
    if form.validate_on_submit():
        user = User.query.filter(User.username == form.password.data).first()
        if user and user.check_password(form.password.data):
            login_user(user, remember=form.remember.data)
            flash('Вход выполнен!', 'alert-success')
            return redirect(url_for('index'))
        else:
            flash('Вход не выполнен!', 'alert-success')
